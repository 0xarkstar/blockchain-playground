  ...
  return (
    <Stack gap="lg">
      <Alert icon={<IconInfoCircle size={16} />} variant="light" color="blue">
        Full SNARK pipeline: Expression → Circuit → R1CS → QAP → Trusted Setup → Proof → Verify.
        This is a simplified simulation over a small field (p=23).
      </Alert>
      ...
      <Paper p="md" withBorder>
        <Stack gap="md">
          ...
          <TextInput
            ...
            value={expression}
            onChange={(e) => { setExpression(e.currentTarget.value); setResult(null); }}
          />
          <Group grow>
      ...
      {result && (
        <>
          <Paper p="md" withBorder>
            <Stack gap="sm">
              <Group justify="space-between">
                ...
                <Badge variant="light" color={result.verified ? "green" : "red"}>
                  {result.verified ? "Proof Verified" : "Verification Failed"}
                </Badge>
              </Group>
              <Text size="sm">
                Output: <Code>{result.output.toString()}</Code> (mod {p.toString()})
              </Text>
            </Stack>
          </Paper>
                ...
                <Table>
                  <Table.Tbody>
                    {Object.entries(step.data).map(([key, val]) => (
                      <Table.Tr key={key}>
                        <Table.Td><Text size="sm">{key}</Text></Table.Td>
                        <Table.Td>
                          <Code>
                            {Array.isArray(val)
                              ? val.join(", ")
          ...
          <Paper p="md" withBorder>
            <Stack gap="sm">
              <Text size="sm" fw={600}>Proof Components</Text>
              <Table>
                <Table.Tbody>
                  <Table.Tr>
                    <Table.Td>π_A</Table.Td>
                    <Table.Td><Code>{result.proof.piA.toString()}</Code></Table.Td>
                  </Table.Tr>
                  <Table.Tr>
                    <Table.Td>π_B</Table.Td>
                    <Table.Td><Code>{result.proof.piB.toString()}</Code></Table.Td>
                  </Table.Tr>
                  <Table.Tr>
                    <Table.Td>π_C</Table.Td>
                    <Table.Td><Code>{result.proof.piC.toString()}</Code></Table.Td>
                  </Table.Tr>
                </Table.Tbody>
              </Table>
            </Stack>
