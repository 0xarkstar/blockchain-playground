...
export function compareStorageVsMemory(operationCount: number): GasComparison {
  ...
  const storageBreakdown: GasBreakdownItem[] = [
    {
      operation: "SSTORE (first write)",
      gasPerOp: SSTORE_SET,
      count: 1,
      totalGas: SSTORE_SET,
    },
    {
      operation: "SSTORE (update)",
      gasPerOp: SSTORE_RESET,
      count: Math.max(0, count - 1),
      totalGas: SSTORE_RESET * Math.max(0, count - 1),
    },
    {
      operation: "SLOAD (read)",
      gasPerOp: SLOAD,
      count,
      totalGas: SLOAD * count,
    },
  ];

  const memoryBreakdown: GasBreakdownItem[] = [
    {
      operation: "MSTORE (write)",
      gasPerOp: MSTORE,
      count,
      totalGas: MSTORE * count,
    },
    {
      operation: "MLOAD (read)",
      gasPerOp: MLOAD,
      count,
      totalGas: MLOAD * count,
    },
  ];
  ...
  return {
    unoptimized: {
      label: "Storage (SSTORE/SLOAD)",
      totalGas: storageTotalGas,
      breakdown: storageBreakdown,
    },
    optimized: {
      label: "Memory (MSTORE/MLOAD)",
      totalGas: memoryTotalGas,
      breakdown: memoryBreakdown,
    },
    savings,
    savingsPercent: storageTotalGas > 0 ? (savings / storageTotalGas) * 100 : 0,
    explanation:
      "Memory operations are orders of magnitude cheaper than storage. Cache storage reads into memory variables when accessed multiple times.",
  };
}
...
export function comparePackedVsUnpacked(fieldCount: number): GasComparison {
  ...
  const unpackedBreakdown: GasBreakdownItem[] = [
    {
      operation: "SSTORE (one per slot)",
      gasPerOp: SSTORE_SET,
      count: unpackedSlots,
      totalGas: SSTORE_SET * unpackedSlots,
    },
  ];
  ...
  const packedBreakdown: GasBreakdownItem[] = [
    {
      operation: "SSTORE (packed slots)",
      gasPerOp: SSTORE_SET,
      count: packedSlots,
      totalGas: SSTORE_SET * packedSlots,
    },
  ];
  ...
  return {
    unoptimized: {
      label: `Unpacked (${unpackedSlots} slots)`,
      totalGas: unpackedTotal,
      breakdown: unpackedBreakdown,
    },
    optimized: {
      label: `Packed (${packedSlots} slot${packedSlots > 1 ? "s" : ""})`,
      totalGas: packedTotal,
      breakdown: packedBreakdown,
    },
    savings,
    savingsPercent: unpackedTotal > 0 ? (savings / unpackedTotal) * 100 : 0,
    explanation:
      "Solidity packs variables smaller than 32 bytes into a single slot if they fit. Ordering variables by size (largest first) maximizes packing.",
  };
}
...
export function compareCallTypes(dataSize: number): GasComparison {
  ...
  const calldataCost =
    nonZeroBytes * CALLDATA_NONZERO_BYTE + zeroBytes * CALLDATA_ZERO_BYTE;

  const callBreakdown: GasBreakdownItem[] = [
    {
      operation: "CALL base cost",
      gasPerOp: CALL_BASE,
      count: 1,
      totalGas: CALL_BASE,
    },
    {
      ...
      gasPerOp: CALL_VALUE_TRANSFER,
      count: 1,
      totalGas: CALL_VALUE_TRANSFER,
    },
    {
      operation: "Calldata",
      gasPerOp: calldataCost,
      count: 1,
      totalGas: calldataCost,
    },
  ];

  const staticCallBreakdown: GasBreakdownItem[] = [
    {
      operation: "STATICCALL base cost",
      gasPerOp: STATICCALL_BASE,
      count: 1,
      totalGas: STATICCALL_BASE,
    },
    {
      operation: "Calldata",
      gasPerOp: calldataCost,
  ...
  return {
    unoptimized: {
      label: "CALL (with value)",
      totalGas: callTotal,
      breakdown: callBreakdown,
    },
    optimized: {
      label: "STATICCALL (read-only)",
      totalGas: staticTotal,
      breakdown: staticCallBreakdown,
    },
    savings,
    savingsPercent: callTotal > 0 ? (savings / callTotal) * 100 : 0,
    explanation:
      "STATICCALL is cheaper because it doesn't allow state modification or value transfer, saving the 9,000 gas value transfer stipend.",
  };
}
...
export function compareMappingVsArray(elementCount: number): GasComparison {
  ...
  const mappingBreakdown: GasBreakdownItem[] = [
    {
      operation: "SLOAD (hash-based lookup)",
      gasPerOp: SLOAD,
      count: 1,
      totalGas: SLOAD,
    },
  ...
  const arrayBreakdown: GasBreakdownItem[] = [
    {
      operation: "SLOAD (length check)",
      gasPerOp: SLOAD,
      count: 1,
      totalGas: SLOAD,
    },
    {
      operation: "SLOAD (element reads)",
      gasPerOp: SLOAD,
      count,
      totalGas: SLOAD * count,
    },
  ...
  return {
    unoptimized: {
      label: `Array scan (${count} elements)`,
      totalGas: arrayTotal,
      breakdown: arrayBreakdown,
    },
    optimized: {
      label: "Mapping lookup (O(1))",
      totalGas: mappingTotal,
      breakdown: mappingBreakdown,
    },
    savings,
    savingsPercent: arrayTotal > 0 ? (savings / arrayTotal) * 100 : 0,
    explanation:
      "Mappings provide O(1) lookups via keccak256 hashing of the key, while iterating an array costs O(n) SLOAD operations.",
  };
}

export function getGasConstantsTable(): readonly {
  name: string;
  gas: number;
  description: string;
}[] {
  return [
    {
      name: "SSTORE (new)",
      gas: SSTORE_SET,
      description: "Store a new value in storage",
    },
    {
      name: "SSTORE (update)",
      gas: SSTORE_RESET,
      description: "Update an existing storage value",
    },
    ...
    {
      name: "CALL + value",
      gas: CALL_BASE + CALL_VALUE_TRANSFER,
      description: "External call with ETH transfer",
    },
    {
      name: "STATICCALL",
      gas: STATICCALL_BASE,
    ...
    {
      name: "DELEGATECALL",
      gas: DELEGATECALL_BASE,
      description: "Delegate call base cost",
    },
    {
      name: "Calldata (zero byte)",
      gas: CALLDATA_ZERO_BYTE,
      description: "Per zero byte in calldata",
    },
    {
      name: "Calldata (non-zero)",
      gas: CALLDATA_NONZERO_BYTE,
      description: "Per non-zero byte in calldata",
    },
  ];
}
